  ...
  el.innerHTML = `
    ...
    <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
      ...
      ${roster.length === 0
        ...
        return `<div class="bg-black/50 rounded-lg px-3 py-2 text-sm ${gold} flex items-center">${w}${badge}</div>`;
      }).join('')}
          </div>`}
    </div>
  `;
            ...
            <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
              ...
              <div class="flex flex-wrap gap-1.5">
                ${(u.roster || []).map(w => {
                  ...
                  return `<span class="text-xs bg-black/60 border rounded px-2 py-1 ${gold}">${w}${badge}</span>`;
...
async function loadChampions() {
  const { data: rows } = await window.supabaseClient.from('champions').select('*');
  data.champions = {};  // name (lowercase) -> short label
  ...
  (rows || []).forEach(r => {
    ...
    // Support comma-separated names for Tag/Trios
