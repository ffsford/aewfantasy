...
async function promptCommissionerPendingDraft() {
  ...
  if (!pending) return;
  // refresh bonuses with latest rosters
...
async function tryAutoBuildDraftForDueShows() {
  ...
  try {
    ...
    if (sb?.functions?.invoke) {
      const { data, error } = await sb.functions.invoke('fetch-show-results', {
        body: { shows: due.map(e => ({ name: e.name, date: e.event_date, type: e.event_type })) }
      });
