...
async function persistClaimRanked() {
  ...
  try {
    if (claimRanked.length === 0) {
      ...
      const drop = (document.getElementById('drop-select')?.value || data.claims[currentUser]?.drop || '').trim() || null;
      await saveClaim(currentUser, [...claimRanked], drop, drop ? [drop] : []);
    }
  } catch (e) {
    console.error('persistClaimRanked', e);
    // Don't alert on every X click if drop not chosen yet — ranked can save without drop column issues
    console.warn('Could not update claim:', e.message || e);
  }
}
...
function renderClaimList() {
  ...
  if (!el) return;
  if (claimRanked.length === 0) {
    el.innerHTML = '<p class="text-sm text-gray-500 text-center py-6">Tap available wrestlers to rank them.<br><span class="text-gray-400">You get <strong class="text-white">ONE</strong> pickup: if #1 is taken, you get #2, then #3, etc. Same drop either way.</span></p>';
    return;
  }
  el.innerHTML = claimRanked.map((w, i) => `
    <div class="claim-item flex items-center justify-between gap-2 py-2 border-b border-gray-800">
      ...
      <button type="button" class="text-red-400 text-lg leading-none px-2 py-1 remove-claim flex-shrink-0" data-idx="${i}" aria-label="Remove">✕</button>
    </div>
  `).join('');
  el.querySelectorAll('.remove-claim').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      renderClaimList();
      await persistClaimRanked();
      const status = document.getElementById('claim-status');
      if (status) {
        status.textContent = claimRanked.length ? 'Claim list updated' : 'Claim cleared';
        status.className = 'text-sm text-center mt-3 text-emerald-400';
      }
    });
  });
}
...
async function processWaivers(opts = {}) {
  ...
  for (const div of ['east', 'west']) {
    ...
    for (const team of order) {
      ...
      // Ranked priority: try #1, if taken try #2, etc. ONE drop either way (Devin rules).
      const drop = claim.drop || (Array.isArray(claim.drops) && claim.drops[0]) || null;
      let gotOne = false;
      for (let ri = 0; ri < claim.ranked.length; ri++) {
        ...
        if (!availableHas(available, wrestler)) {
          results.push(`↪️ ${team.name} priority #${ri + 1} ${wrestler} already taken — trying next`);
          continue;
        }
        ...
        if (drop && rosterHas(u.roster, drop)) {
          ...
          await clearClaim(team.pin);
          results.push(`✅ ${team.name} claimed <strong>${claimName}</strong> (priority #${ri + 1}, dropped ${drop})`);
          gotOne = true;
          break;
        } else if (u.roster.length < maxR) {
          u.roster.push(claimName);
          await saveTeamRoster(team.pin, u.roster);
          await clearClaim(team.pin);
          results.push(`✅ ${team.name} claimed <strong>${claimName}</strong> (priority #${ri + 1})`);
          gotOne = true;
          break;
        } else {
          results.push(`⚠️ ${team.name} could claim ${claimName} but drop "${drop || ''}" not on roster / roster full`);
          break;
        }
      }
      if (!gotOne && claim.ranked?.length) {
        results.push(`❌ ${team.name} — no ranked wrestler available (or drop invalid)`);
        await clearClaim(team.pin);
      }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('submit-claim')?.addEventListener('click', async () => {
    if (claimRanked.length === 0) {
      document.getElementById('claim-status').textContent = 'Add at least one wrestler to your ranked list';
      document.getElementById('claim-status').className = 'text-sm text-center mt-3 text-red-400';
      return;
    }
    const drop = (document.getElementById('drop-select')?.value || '').trim();
    if (!drop) {
      document.getElementById('claim-status').textContent = 'Select ONE wrestler to drop (used no matter which ranked pick you get)';
      document.getElementById('claim-status').className = 'text-sm text-center mt-3 text-red-400';
      return;
    }
    try {
      await saveClaim(currentUser, [...claimRanked], drop, [drop]);
      const sealed = claimsAreSealed();
      document.getElementById('claim-status').textContent = sealed
        ? 'Claim saved & locked until ' + new Date(data.waiverDeadline).toLocaleString() + '. Priority order: if #1 taken → #2, etc. Drop: ' + drop
        : 'Claim saved. Priority order: if #1 taken → #2. Drop: ' + drop;
      ...
      document.getElementById('claim-status').className = 'text-sm text-center mt-3 text-red-400';
      alert('Claim failed to save:\n' + (e.message || e) + '\n\nIf this mentions drop column, run the SQL Ford has for claims.drop');
    }
  });
