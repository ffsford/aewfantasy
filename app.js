...
function renderWaiver() {
  ...
  if (available.length === 0) {
    ...
    container.innerHTML = available.map(w => {
      const pts = getWrestlerPts(w);
      return `<div class="wrestler-chip px-3 py-2 rounded-lg text-sm border border-transparent hover:border-gray-600 flex justify-between gap-2" data-name="${w}">
        <span>${w}</span>
        <span class="text-gray-500 font-mono text-xs">${pts}</span>
      </div>`;
    }).join('');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('wp-bulk-btn')?.addEventListener('click', async () => {
    ...
    await loadWrestlerPoints(); // confirm from DB
    if (msg) {
      if (result.ok) {
