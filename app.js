  ...
  list.innerHTML = events.map(ev => {
    ...
    return `<div class="flex justify-between gap-2 items-center bg-black/40 rounded-lg px-3 py-2">
      <div class="min-w-0">
        ...
        <div class="text-xs text-gray-500">${CAL_TYPES[ev.event_type] || ev.event_type} · ${ev.frequency || ''}${extra ? ' · ' + extra : ''}</div>
      </div>
      <button type="button" data-cal-del="${ev.id}" class="text-red-400 text-xs flex-shrink-0">Delete</button>
    </div>`;
  }).join('');
  list.querySelectorAll('[data-cal-del]').forEach(btn => {
    btn.onclick = async () => {
      if (!confirm('Delete this event?')) return;
      await deleteCalendarEvent(btn.dataset.calDel);
      renderCalendarAdmin();
      renderCalendar();
    };
  });
    ...
    el.innerHTML = list.map((t, i) => {
      ...
      <div class="standing-row ${t.pin === currentUser ? 'me' : ''}">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <span class="font-semibold truncate">${t.name}${champCrown(t.pin)}</span>
  ...
  el.innerHTML = `
    ...
    <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
      ...
      ${roster.length === 0
        ...
        return `<div class="bg-black/50 rounded-lg px-3 py-2 text-sm ${gold} flex items-center gap-2">
...
async function enterApp() {
  ...
  if ((tab === 'commissioner' || tab === 'broadcast') && !u.isCommissioner) tab = 'standings';
            ...
            <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
              ...
              <div class="flex flex-wrap gap-1.5">
                ${(u.roster || []).slice().sort((a, b) => getWrestlerPts(b) - getWrestlerPts(a) || a.localeCompare(b)).map(w => {
                  ...
                  const pts = getWrestlerPts(w);
                  return `<span class="text-xs bg-black/60 border rounded pl-1 pr-2 py-1 ${gold} inline-flex items-center gap-1.5">
                    ${portraitHtml(w, 'h-20 w-20')}
                    <span>${w}${badge}</span>
...
function renderScoreInputs() {
  ...
  el.innerHTML = teams.map(([pin, u]) => {
    ...
    <div class="flex items-center gap-2 flex-wrap">
      ...
      <input type="number" data-pin="${pin}" data-current="${cur}" class="score-add-input w-24 bg-black border border-gray-700 rounded-lg px-2 py-1.5 text-sm" placeholder="New total" />
    </div>`;
  }).join('');
}
...
async function saveScores() {
  ...
  // Inputs are NEW TOTALS. Blank = leave team alone.
  ...
  // Reload latest points from DB so deltas are accurate
  try {
    const { data: teams } = await window.supabaseClient.from('teams').select('pin, points');
    (teams || []).forEach(t => { data.points[t.pin] = t.points || 0; });
  } catch (e) {
    console.error('Could not refresh points before save', e);
  }

  // Clear previous show deltas for teams not updated this save
  try {
    const allPins = Object.keys(data.users).filter(p => data.users[p].division);
    const scoredPins = new Set(scored.map(s => s.pin));
    for (const pin of allPins) {
      if (scoredPins.has(pin)) continue;
      await window.supabaseClient.from('teams').update({ last_delta: null }).eq('pin', pin);
      if (data.users[pin]) data.users[pin].lastDelta = null;
    }
  } catch (e) {
    console.warn('Could not clear last_delta', e);
  }

  for (const { pin, newTotal } of scored) {
