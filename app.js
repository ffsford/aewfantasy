...
async function tryAutoLockDraftOrders() {
  ...
  for (const div of getLeagueDivisions()) {
    try {
      ...
      if (draft.order_pins && draft.order_pins.length) continue; // already locked
      await generateDraftOrder(div);
      console.log('Auto-locked draft order for', div);
    } catch (e) {
...
async function generateDraftOrder(division) {
  if (guardReadOnly('change draft order')) return;
...
async function renderDraft() {
  ...
  if (pickArea) {
    if (d?.status === 'active' && d.order_pins?.length) {
      ...
      pickArea.innerHTML = `
        <div class="text-center mb-4">
          <div class="text-lg font-bold">${divisionLabel(focusDiv)} — not started</div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (savedPin && data.users[savedPin]) {
    ...
    try { enterApp(); } catch (e) { console.error(e); alert('Logged in but the app failed to open: ' + (e.message || e)); }
  ...
  document.getElementById('div-rand-run')?.addEventListener('click', randomizeDivisions);
