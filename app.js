...
async function saveScores() {
  ...
  // Apply all: teams that scored get their delta; others reset delta to 0 so old +N clears
  for (const [pin, add] of Object.entries(deltas)) {
    const current = data.points[pin] || 0;
    const next = current + (add || 0);
    await saveTeamPoints(pin, next, add || 0);
  }
