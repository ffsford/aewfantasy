...
async function activateForeignObject() {
  ...
  if (!confirm('Clear ' + foName + ' from ' + name + '?\n\nThis removes it from Currently Held. Use this after you already applied the effect by hand.')) return;
  const res = await removeHeldFO(pin, type);
  if (!res.ok) {
    alert('Could not clear FO: ' + res.error);
    return;
  }
  if (log) {
    const row = document.createElement('div');
    row.textContent = 'Cleared ' + foName + ' from ' + name;
    log.prepend(row);
  }
  renderForeignObjects();
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('fo-activate-player')?.addEventListener('change', fillActivateFOTypes);
