...
function renderStandings() {
  const east = Object.entries(data.users)
    ...
    .map(([pin, u]) => ({ pin, name: u.name, points: data.points[pin] || 0, maxRoster: u.maxRoster, lastDelta: u.lastDelta || 0 }))
  ...
  const renderList = (list, containerId) => {
    ...
    el.innerHTML = list.map((t, i) => {
      const delta = data.users[t.pin]?.lastDelta || 0;
      const deltaHtml = delta > 0
        ? `<span class="text-emerald-400 text-sm font-semibold ml-2">+${delta}</span>`
        : delta < 0
          ? `<span class="text-red-400 text-sm font-semibold ml-2">${delta}</span>`
          : '';
      return `
...
async function saveScores() {
  ...
  // Only touch teams where Devin typed a number. Never rewrite points from blank boxes.
  const scored = [];
  for (const input of inputs) {
  ...
  // Reload latest points from DB first so we never add onto a stale 0
  try {
  ...
  // Set last_delta = 0 for everyone first (clear old green numbers), without changing points
  try {
    const allPins = Object.keys(data.users).filter(p => data.users[p].division);
    for (const pin of allPins) {
      await window.supabaseClient.from('teams').update({ last_delta: 0 }).eq('pin', pin);
      if (data.users[pin]) data.users[pin].lastDelta = 0;
    }
  } catch (e) {
    console.warn('Could not clear last_delta', e);
  }

  // Then apply only the scored teams
  for (const { pin, add } of scored) {
    const current = data.points[pin] || 0;
    const next = current + add;
    await saveTeamPoints(pin, next, add);
  }
