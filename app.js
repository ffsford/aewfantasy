...
function portraitUrl(name) {
  ...
  // 3) Auto: any file in /portraits/ named exactly like the wrestler
  //    e.g. portraits/Kenny Omega.png
  return 'portraits/' + encodeURIComponent(name.trim()) + '.png';
}
...
function portraitHtml(name, sizeClass) {
  ...
  if (url) {
    return `<img src="${url}" alt="" class="${sz} rounded object-cover flex-shrink-0 border border-gray-700 bg-black" onerror="this.style.display='none';this.nextElementSibling&&this.nextElementSibling.classList.remove('hidden')" />` +
      `<div class="hidden ${sz} rounded bg-gray-800 flex items-center justify-center text-[10px] font-bold text-gray-500 flex-shrink-0 border border-gray-700">${(name||'?').split(/\\s+/).map(p=>p[0]||'').join('').slice(0,2).toUpperCase()}</div>`;
  }
