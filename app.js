...
const SCORING_BONUSES = {
  ...
  // Dynamite #361 (2026-09-02) live examples:
  //   Cliff: Swerve + Creed same side (Trios) → tag +2
  //   Evan: Ospreay vs Connors (5v5) → rivalry +2
  //   Bito: Andrade vs Ricochet (5v5) → rivalry +2
  //   Brian: Kidd + Liona + Kaun same side (5v5) → tag +3
  //   Brian: Bayne vs Windsor → rivalry +2
  //   Totes: Kidd + Liona + Kaun same side (5v5) → tag +3
  multiPersonEnding: 1,
  tournamentOpening: 1,
...
const WRESTLER_ALIASES = {
  'kofi kingston': 'kofi',
  'austin creed': 'austin creed',
  'andrade el idolo': 'andrade',
  'andrade el ídolo': 'andrade',
  'will ospreay': 'will ospreay',
  'dezmond xavier': 'dezmond xavier',
  'zachary wentz': 'zachary wentz'
};

function canonWrestlerName(n) {
  const k = String(n || '').toLowerCase().trim().replace(/\s+/g, ' ');
  return WRESTLER_ALIASES[k] || k;
}

function rosterNameSet(roster) {
  return new Set((roster || []).map(canonWrestlerName).filter(Boolean));
}
...
function computeTagTeamBonus(roster, sides) {
  const rosterSet = rosterNameSet(roster);
  let points = 0;
  const breakdown = [];
  (sides || []).forEach((side, idx) => {
    const mine = (side || []).filter(w => rosterSet.has(canonWrestlerName(w)));
    if (mine.length >= 2) {
      points += mine.length;
...
/** Dynamite #361 fixtures — used when building an import draft */
const DYNAMITE_361_SIDES = {
  trios: [
    ['Swerve Strickland', 'Kofi', 'Austin Creed'],
    ['Dezmond Xavier', 'Myron Reed', 'Zachary Wentz']
  ],
  tnt: [['Darby Allin'], ['Mark Davis']],
  bayne: [['Megan Bayne'], ['Alex Windsor']],
  mox: [['Jon Moxley'], ['AR Fox']],
  main: [
    ['Will Ospreay', 'Andrade', 'Callum Newman', 'HENARE', 'Francesco Akira'],
    ['Gabe Kidd', 'Clark Connors', 'Ricochet', 'Bishop Kaun', 'Toa Liona']
  ]
};
...
function prettyWrestlerName(raw) {
  const t = String(raw || '').replace(/\s+/g, ' ').trim();
  if (!t) return '';
  const key = t.toLowerCase();
  const SPECIAL = {
    'mjf': 'MJF', 'pac': 'PAC', 'ar fox': 'AR Fox', 'thekla': 'Thekla',
    'hangman page': 'Hangman Page', 'adam page': 'Hangman Page',
    'mercedes mone': 'Mercedes Mone', "kyle o'reilly": "Kyle O'Reilly",
    'kyle orielly': "Kyle O'Reilly", 'kommander': 'Komander', 'komander': 'Komander',
    'zack sabre jr': 'Zack Sabre Jr', 'will ospreay': 'Will Ospreay',
    'francesco akira': 'Francesco Akira', 'henare': 'HENARE', 'mistico': 'Mistico',
    'ilja dragunov': 'Ilja Dragunov', 'vertvixen': 'VertVixen', 'viva van': 'Viva Van',
    'drilla maloney': 'Drilla Maloney', 'tommaso ciampa': 'Tommaso Ciampa',
    'kofi kingston': 'Kofi', 'kofi': 'Kofi', 'jd drake': 'JD Drake',
    'el phantasmo': 'El Phantasmo', 'el clon': 'El Clon',
    'mascara dorada': 'Mascara Dorada', 'starlight kid': 'Starlight Kid'
  };
  if (SPECIAL[key]) return SPECIAL[key];
  return t.toLowerCase().replace(/\b([a-z])/g, m => m.toUpperCase()).replace(/O'Reilly/i, "O'Reilly");
}

async function saveMasterRoster(names) {
  const cleaned = [...new Set((names || []).map(prettyWrestlerName).filter(Boolean))];
  if (cleaned.length < 8) {
    throw new Error('Refusing to save — list has ' + cleaned.length + ' names. That would wipe the roster. Use Add wrestler for 1–2 names.');
  }
  const existing = data.masterRoster || [];
  if (cleaned.length < existing.length - 5) {
    const ok = confirm('This will REPLACE the current ' + existing.length + ' wrestlers with ' + cleaned.length + '. Continue?');
...
function renderStandings() {
  try { paintCommishTradeNotice(); } catch (e) {}
  ...
  if (available.length === 0) {
    ...
    container.querySelectorAll('.wrestler-chip').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (typeof waiverDeadlinePassed === 'function' && waiverDeadlinePassed()) {
          alert('Waiver deadline has passed. Claims are locked.');
          return;
        }
...
async function rescindEntireClaim() {
  if (!currentUser || isReadOnlyViewer()) return;
  if (typeof waiverDeadlinePassed === 'function' && waiverDeadlinePassed()) {
    alert('Waiver deadline has passed. Claims are locked.');
    return;
  }
  if (!claimRanked.length && !(claimDrops || []).filter(Boolean).length && !data.claims[currentUser]) {
    alert('You do not have a saved claim.');
    return;
  }
  if (!confirm('Rescind your entire waiver claim? All pickups and drops will be deleted.')) return;
  claimRanked = [];
  claimDrops = [];
  try {
    await clearClaim(currentUser);
  } catch (e) {
    alert('Could not rescind: ' + (e.message || e));
    return;
  }
  renderWaiver();
  const msg = document.getElementById('rescind-claim-msg');
  if (msg) {
    msg.textContent = 'Claim rescinded.';
...
function renderDropPool() {
  ...
  el.innerHTML = `
    ...
    <div class="space-y-1 mb-2" id="drop-pool-chips">
      ${claimDrops.filter(Boolean).length ? claimDrops.filter(Boolean).map((d, i) => `
        <div class="drop-item flex items-center justify-between gap-2 py-2 px-2 rounded-lg bg-gray-800/80 border border-gray-700 cursor-grab" data-idx="${i}">
          <div class="flex items-center gap-2 min-w-0">
            <span class="text-gray-500 select-none">⠿</span>
            <span class="text-aew-gold font-bold text-sm w-6">${i + 1}.</span>
            <span class="truncate text-sm">${d}</span>
          </div>
          <div class="flex items-center gap-1">
            <button type="button" class="move-drop px-2 py-1 text-xs bg-black rounded" data-idx="${i}" data-dir="-1" ${i===0?'disabled':''}>↑</button>
            <button type="button" class="move-drop px-2 py-1 text-xs bg-black rounded" data-idx="${i}" data-dir="1" ${i===claimDrops.filter(Boolean).length-1?'disabled':''}>↓</button>
            <button type="button" class="text-red-400 remove-drop-pool px-2" data-idx="${i}">✕</button>
          </div>
        </div>
  ...
  el.querySelectorAll('.move-drop').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      const live = claimDrops.filter(Boolean);
      const i = parseInt(btn.getAttribute('data-idx'), 10);
      const dir = parseInt(btn.getAttribute('data-dir'), 10);
      moveArrayItem(live, i, i + dir);
      claimDrops = live;
      renderDropPool();
      renderClaimPreview();
      persistClaimRanked();
  ...
  wireReorderList(el.querySelector('#drop-pool-chips'), '.drop-item', (from, to) => {
    const live = claimDrops.filter(Boolean);
    moveArrayItem(live, from, to);
    claimDrops = live;
    renderDropPool();
    renderClaimPreview();
    persistClaimRanked();
...
function wireReorderList(container, itemSelector, onDropIndex) {
  if (!container) return;
  let dragFrom = null;
  container.querySelectorAll(itemSelector).forEach(row => {
    ...
    row.addEventListener('dragover', (e) => { e.preventDefault(); row.classList.add('bg-aew-gold/10'); });
    row.addEventListener('dragleave', () => row.classList.remove('bg-aew-gold/10'));
    row.addEventListener('drop', (e) => {
      e.preventDefault();
      row.classList.remove('bg-aew-gold/10');
...
function renderClaimList() {
  ...
  if (claimRanked.length === 0) {
    el.innerHTML = '<p class="text-sm text-gray-500 text-center py-6">Tap wrestlers you want. Drag to reorder. Use Rescind on a name or Rescind entire claim below before the deadline.</p>';
  } else {
    el.innerHTML = `<p class="text-[11px] text-gray-500 mb-2">Hold and drag a name onto another to change priority. Arrows work on phone.</p>` + claimRanked.map((w, i) => `
      <div class="claim-item flex items-center justify-between gap-2 py-2 border-b border-gray-800 cursor-grab active:cursor-grabbing" data-idx="${i}">
        <div class="flex items-center gap-2 min-w-0">
          <span class="text-gray-500 text-lg leading-none select-none" title="Drag">⠿</span>
          <span class="text-aew-gold font-bold text-sm w-6">${i + 1}.</span>
          ${portraitHtml(w, 'h-14 w-14')}
          <span class="truncate font-medium">${w}</span>
        </div>
        <div class="flex items-center gap-1 flex-shrink-0">
          <button type="button" class="move-claim px-2 py-1 text-xs bg-gray-800 rounded" data-idx="${i}" data-dir="-1" ${i===0?'disabled':''}>↑</button>
          <button type="button" class="move-claim px-2 py-1 text-xs bg-gray-800 rounded" data-idx="${i}" data-dir="1" ${i===claimRanked.length-1?'disabled':''}>↓</button>
          <button type="button" class="remove-claim text-red-400 text-xs font-bold px-2 py-1 border border-red-800 rounded" data-idx="${i}">Rescind</button>
        </div>
      </div>
    `).join('');
  }
  ...
  el.querySelectorAll('.remove-claim').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (Array.isArray(claimDrops) && i < claimDrops.length) claimDrops.splice(i, 1);
      renderClaimList();
      persistClaimRanked();
    });
  });
  el.querySelectorAll('.move-claim').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      const i = parseInt(btn.getAttribute('data-idx'), 10);
      const dir = parseInt(btn.getAttribute('data-dir'), 10);
      moveArrayItem(claimRanked, i, i + dir);
      renderClaimList();
      persistClaimRanked();
    });
  });
  wireReorderList(el, '.claim-item', (from, to) => {
    moveArrayItem(claimRanked, from, to);
    renderClaimList();
    persistClaimRanked();
  });
  renderDropPool();
...
function renderCommissioner() {
  try { renderCommishTradeQueue(); } catch (e) {}
  ...
  if (heldEl) {
    ...
    Object.entries(data.foreignObjects || {}).forEach(([pin, list]) => {
      ...
      if (!u) return;
      list.forEach(fo => {
        entries.push(`<div class="flex justify-between items-center gap-2">
          <span>${u.name}</span>
          <span class="text-aew-gold">${fo.name}${fo.banked ? ' (banked)' : ''}</span>
          <button type="button" class="fo-clear-held text-[10px] bg-red-800 hover:bg-red-700 text-white font-bold px-2 py-0.5 rounded" data-pin="${pin}" data-fo="${fo.id}">Clear</button>
        </div>`);
      });
    });
    heldEl.innerHTML = entries.length ? entries.join('') : '<p class="text-gray-500">None awarded yet</p>';
  }
  if (typeof fillActivateFOTypes === 'function') fillActivateFOTypes();
...
let lastPlayedWheelId = null;

function publishWheelSpin(payload) {
  return window.supabaseClient.from('settings').upsert({
    key: 'wheel_live_spin',
    value: JSON.stringify(payload)
  });
}

async function broadcastWheelResult(region, label) {
  const msg = 'WHEEL OF BOOM — ' + String(region || '').toUpperCase() + '\nResult: ' + String(label || '').replace(/\n/g, ' ');
  try {
    ...
    console.warn('wheel broadcast', e);
  }
}

function playWheelOnCanvas(canvas, targetRotation, durationMs, onDone) {
  if (!canvas) { if (onDone) onDone(); return; }
  drawWheelOn(canvas);
  canvas.style.transition = 'none';
  canvas.style.transform = 'rotate(0deg)';
  void canvas.offsetWidth;
  canvas.style.transition = 'transform ' + durationMs + 'ms cubic-bezier(0.12, 0.7, 0.1, 1)';
  canvas.style.transform = 'rotate(' + targetRotation + 'deg)';
  setTimeout(() => { if (onDone) onDone(); }, durationMs + 50);
}

function showWheelLiveModal(ev, isReplay) {
  const modal = document.getElementById('wheel-live-modal');
  const sub = document.getElementById('wheel-live-sub');
  const res = document.getElementById('wheel-live-result');
  const canvas = document.getElementById('wheel-live-canvas');
  if (!modal || !canvas) return;
  modal.classList.remove('hidden');
  if (sub) sub.textContent = (isReplay ? 'Replay — ' : 'LIVE — ') + String(ev.region || '').toUpperCase();
  if (res) res.textContent = ev.spinning ? 'Spinning...' : (ev.label || '');
  const elapsed = ev.startedAt ? Date.now() - new Date(ev.startedAt).getTime() : 99999;
  const dur = ev.durationMs || 4200;
  if (ev.spinning && elapsed < dur) {
    const remain = Math.max(400, dur - elapsed);
    playWheelOnCanvas(canvas, ev.targetRotation, remain, () => {
      if (res) res.textContent = ev.label || '';
    });
  } else {
    drawWheelOn(canvas);
    canvas.style.transition = 'none';
    canvas.style.transform = 'rotate(' + (ev.targetRotation || 0) + 'deg)';
    if (res) res.textContent = ev.label || '';
  }
}

async function pollWheelLive() {
  if (!window.supabaseClient || !currentUser) return;
  try {
    const { data: row } = await window.supabaseClient.from('settings').select('value').eq('key', 'wheel_live_spin').maybeSingle();
    if (!row?.value) return;
    const ev = typeof row.value === 'string' ? JSON.parse(row.value) : row.value;
    if (!ev || !ev.id) return;
    if (ev.id === lastPlayedWheelId) return;
    lastPlayedWheelId = ev.id;
    showWheelLiveModal(ev, false);
  } catch (e) {}
}

function replayLastWheel() {
  pollWheelLiveReplay();
}

async function pollWheelLiveReplay() {
  try {
    const { data: row } = await window.supabaseClient.from('settings').select('value').eq('key', 'wheel_live_spin').maybeSingle();
    if (!row?.value) return alert('No spin to replay yet.');
    const ev = typeof row.value === 'string' ? JSON.parse(row.value) : row.value;
    lastPlayedWheelId = null;
    ev.spinning = true;
    ev.startedAt = new Date().toISOString();
    showWheelLiveModal(ev, true);
    lastPlayedWheelId = ev.id;
  } catch (e) {
    alert('Could not load last spin');
  }
}

function spinWheel() {
  if (typeof guardReadOnly === 'function' && guardReadOnly('spin the Wheel of Boom')) return;
  ...
  const label = String(result.label || '').replace(/\n/g, ' ');
  const ev = {
    id: 'w' + Date.now(),
    spinning: true,
    region,
  ...
  lastPlayedWheelId = ev.id;
  publishWheelSpin(ev);
  playWheelOnCanvas(canvas, targetRotation, 4200, () => {
    document.getElementById('wheel-result').innerHTML =
      '<span class="text-aew-gold">' + label + '</span><br><span class="text-sm text-gray-400">' + region.toUpperCase() + ' — sent live to the league</span>';
    isSpinning = false;
    ev.spinning = false;
    publishWheelSpin(ev);
    broadcastWheelResult(region, label);
    const victim = lastPlacePin(region);
    if (victim) {
      applyWheelSegment(victim, result).then((msg) => {
        const el = document.getElementById('wheel-result');
        if (el) el.innerHTML += '<br><span class="text-sm text-emerald-400">' + msg + '</span>';
        foLog(msg);
      }).catch((e) => alert('Wheel apply failed: ' + (e.message || e)));
    }
  });
}
...
async function enterApp() {
  ...
  loadPendingBroadcasts(); pollWheelLive();
  let tab = 'standings';
          ...
          ${teams.map(([pin, u]) => `
            <div class="bg-aew-card rounded-xl border border-gray-800 p-4">
              <div class="flex justify-between items-center mb-2">
                <span class="min-w-0">
                  <span class="font-semibold ${u.purged ? 'line-through text-red-400/80' : ''}" title="Owner: ${teamOwnerLabel(pin)}">${teamDisplayName(pin)}</span>
                  ${u.purged ? '<span class="ml-2 text-[10px] font-bold tracking-wide text-red-400 border border-red-500/60 rounded px-1.5 py-0.5">PURGED</span>' : ''}
                  ${!u.purged && teamDisplayName(pin) !== teamOwnerLabel(pin) ? `<span class="block text-[10px] text-gray-500" title="Owner: ${teamOwnerLabel(pin)}">${teamOwnerLabel(pin)}</span>` : ''}
                </span>
                <span class="text-sm text-gray-400">${u.purged ? 'PURGED' : ((u.roster || []).length + '/' + u.maxRoster)} • ${data.points[pin] || 0} pts</span>
              </div>
...
function renderCommishTradeQueue() {
  const el = document.getElementById('commish-trade-queue');
  if (!el) return;
  if (!data.users[currentUser]?.isCommissioner) {
    el.innerHTML = '';
    return;
  }
  const list = (data.pendingTrades || []).filter(t =>
    t.status === 'accepted_by_player' || t.status === 'accepted' || t.status === 'pending'
  );
  if (!list.length) {
    el.innerHTML = '<p class="text-gray-500 text-sm">No trades waiting</p>';
    return;
  }
  el.innerHTML = list.map(t => {
    const a = data.users[t.from_pin]?.name || t.from_pin;
    const b = data.users[t.to_pin]?.name || t.to_pin;
    const ready = t.status !== 'pending';
    const label = ready ? 'Both owners accepted — approve or veto' : 'Offer sent — other owner has not accepted in the system yet';
    const tid = String(t.id).replace(/'/g, "\\'");
    return `<div class="border border-gray-700 rounded-lg p-3 mb-2">
      <p class="font-semibold">${a} ↔ ${b}</p>
      <p class="text-sm text-gray-300">${a} sends: ${(t.offering || []).join(', ') || '—'}</p>
      <p class="text-sm text-gray-300">${b} sends: ${(t.requesting || []).join(', ') || '—'}</p>
      <p class="text-xs ${ready ? 'text-aew-gold' : 'text-gray-500'} mt-1">${label}</p>
      <div class="flex gap-2 mt-2">
        <button type="button" onclick="resolveTrade('${tid}', true)" class="bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold px-3 py-1.5 rounded-lg">Approve</button>
        <button type="button" onclick="resolveTrade('${tid}', false)" class="bg-red-700 hover:bg-red-600 text-white text-sm px-3 py-1.5 rounded-lg">Veto</button>
      </div>
    </div>`;
  }).join('');
}
...
function paintCommishTradeNotice() {
  const waiting = (data.pendingTrades || []).filter(t =>
    t.status === 'accepted_by_player' || t.status === 'accepted' || t.status === 'pending'
  );
  const isCommish = !!(currentUser && data.users[currentUser]?.isCommissioner);
  let el = document.getElementById('commish-trade-standings');
  if (!el) {
    const host = document.getElementById('tab-standings');
    if (host) {
      el = document.createElement('div');
      el.id = 'commish-trade-standings';
      host.insertBefore(el, host.firstChild);
    }
  }
  if (!el) return;
  if (!isCommish || !waiting.length) {
    el.className = 'hidden';
    el.innerHTML = '';
    return;
  }
  el.className = 'mb-4 p-3 rounded-xl border border-aew-gold/50 bg-aew-gold/10';
  el.innerHTML = '<p class="font-bold text-aew-gold mb-1">Trades need your approval</p>' +
    waiting.map(t => {
      const a = (data.users[t.from_pin] && data.users[t.from_pin].name) || t.from_pin;
      const b = (data.users[t.to_pin] && data.users[t.to_pin].name) || t.to_pin;
      const tid = String(t.id).replace(/'/g, "\\'");
      return '<div class="text-sm text-gray-200 mb-2">' + a + ' ↔ ' + b +
        '<div class="text-xs text-gray-400">' + a + ' sends ' + ((t.offering||[]).join(', ')||'—') +
        ' · ' + b + ' sends ' + ((t.requesting||[]).join(', ')||'—') + '</div>' +
        '<button type="button" class="mt-1 bg-emerald-600 text-white text-xs font-bold px-3 py-1 rounded" onclick="resolveTrade(\'' + tid + '\', true)">Approve</button> ' +
        '<button type="button" class="mt-1 bg-red-700 text-white text-xs px-3 py-1 rounded" onclick="resolveTrade(\'' + tid + '\', false)">Veto</button></div>';
    }).join('');
}
...
async function loadTrades() {
  const { data: trades } = await window.supabaseClient
    .from('trades')
    .select('*')
    .in('status', ['pending', 'accepted_by_player', 'accepted']);
...
async function resolveTrade(tradeId, accept) {
  ...
  if (accept) {
    if (isCommish && (trade.status === 'accepted_by_player' || trade.status === 'accepted' || trade.status === 'pending')) {
    ...
    let { error: accErr } = await window.supabaseClient.from('trades')
      .update({ status: 'accepted_by_player' })
      .eq('id', tradeId);
    if (accErr) {
      const retry = await window.supabaseClient.from('trades')
        .update({ status: 'accepted' })
        .eq('id', tradeId);
      accErr = retry.error;
    }
    if (accErr) {
      alert('Accept did not save: ' + accErr.message + '\nTell Devin — the trade is still pending.');
      return;
    }
    ...
    const descP = `Giving: ${(trade.offering || []).join(', ')} | Requesting: ${(trade.requesting || []).join(', ')}`;
    ...
    const commishPins = Object.entries(data.users).filter(([, u]) => u && u.isCommissioner).map(([p]) => p);
    for (const cp of commishPins) {
      if (cp === currentUser) continue;
      await notifyTradeUpdate(cp, `RED ALERT: TRADE NEEDS YOUR APPROVAL\n${toNameP} accepted a trade.\n${data.users[trade.from_pin]?.name || trade.from_pin} ↔ ${data.users[trade.to_pin]?.name || trade.to_pin}\n${descP}\nOpen Commissioner → Trades waiting for you.`);
    }
...
renderLeagueRosters = function() {
  ...
  const incoming = (data.pendingTrades || []).filter(t => 
    isCommish ? (t.status === 'accepted_by_player' || t.status === 'accepted' || t.status === 'pending' || t.to_pin === currentUser) : t.to_pin === currentUser
  );
  ...
  if (incoming.length || outgoing.length) {
    ...
    incoming.forEach(t => {
      ...
      const isCommish = data.users[currentUser]?.isCommissioner;
      const waitingForCommish = t.status === 'accepted_by_player' || t.status === 'accepted' || (isCommish && t.status === 'pending');
      ...
      <div>
        ...
        <div class="space-y-3">
          ${teams.map(([pin, u]) => {
            ...
            const isPurged = !!u.purged;
            const canTrade = !isMe && sameDiv && !isPurged;
            const nm = teamDisplayName(pin);
            return `
            <div class="bg-aew-card rounded-xl border ${isPurged ? 'border-red-900/60' : 'border-gray-800'} p-4 ${isPurged ? 'opacity-80' : ''}">
              <div class="flex justify-between items-center mb-2">
                <span class="min-w-0">
                  <span class="font-semibold ${isPurged ? 'line-through text-red-400/80' : ''}" title="Owner: ${teamOwnerLabel(pin)}">${nm}${isMe ? ' (you)' : ''}</span>
                  ${isPurged ? '<span class="ml-2 text-[10px] font-bold tracking-wide text-red-400 border border-red-500/60 rounded px-1.5 py-0.5 align-middle">PURGED</span>' : ''}
                </span>
                <div class="flex items-center gap-2">
                  <span class="text-sm text-gray-400">${isPurged ? '0/0' : ((u.roster || []).length + '/' + u.maxRoster)} • ${data.points[pin] || 0} pts</span>
...
function fillActivateFOTypes() {
  const pinSel = document.getElementById('fo-activate-player');
  const typeSel = document.getElementById('fo-activate-type');
  if (!typeSel) return;
  const pin = pinSel ? pinSel.value : '';
  const list = (pin && data.foreignObjects && data.foreignObjects[pin]) ? data.foreignObjects[pin] : [];
  if (!pin) {
    typeSel.innerHTML = '<option value="">Select a player first...</option>';
    return;
  }
  if (!list.length) {
    typeSel.innerHTML = '<option value="">No FO held by this player</option>';
    return;
  }
  typeSel.innerHTML = '<option value="">Select FO to activate / clear...</option>' +
    list.map(fo => {
      const id = fo.id || fo.fo_type;
      const label = (fo.name || id) + (fo.banked ? ' (banked)' : '');
      return '<option value="' + String(id).replace(/"/g, '&quot;') + '">' + label + '</option>';
    }).join('');
}

async function removeHeldFO(pin, foType) {
  if (!pin || !foType) return { ok: false, error: 'missing' };
  const { error } = await window.supabaseClient.from('foreign_objects')
    .delete()
    .eq('pin', pin)
    .eq('fo_type', foType);
  if (error) return { ok: false, error: error.message };
  const list = data.foreignObjects[pin] || [];
  data.foreignObjects[pin] = list.filter(fo => fo.id !== foType);
  if (!data.foreignObjects[pin].length) delete data.foreignObjects[pin];
  return { ok: true };
}

function foLog(msg) {
  const log = document.getElementById('fo-activate-log');
  if (!log) return;
...
function lastPlacePin(division) {
  const rows = Object.entries(data.users || {})
    .filter(([p, u]) => u && u.division === division && !u.purged);
  rows.sort((a, b) => (data.points[a[0]] || 0) - (data.points[b[0]] || 0));
  return rows[0] ? rows[0][0] : null;
}

async function addTeamPoints(pin, delta) {
  const cur = data.points[pin] || 0;
  const next = cur + delta;
  await window.supabaseClient.from('teams').update({ points: next, last_delta: delta }).eq('pin', pin);
  data.points[pin] = next;
  if (data.users[pin]) data.users[pin].lastDelta = delta;
}

async function applyWheelSegment(pin, result) {
  const u = data.users[pin];
  if (!u) throw new Error('No player');
  const div = u.division;
  if (result.effect === 'points') {
    await addTeamPoints(pin, result.value);
    return (u.name || pin) + ' +' + result.value + ' points';
  }
  if (result.effect === 'roster9') {
    await window.supabaseClient.from('teams').update({ max_roster: 9 }).eq('pin', pin);
    u.maxRoster = 9;
    return (u.name || pin) + ' roster size set to 9';
  }
  if (result.effect === 'steal5') {
    const others = Object.entries(data.users)
      .filter(([p, x]) => p !== pin && x.division === div && !x.purged);
    for (const [p] of others) await addTeamPoints(p, -5);
    const gain = 5 * others.length;
    await addTeamPoints(pin, gain);
    return (u.name || pin) + ' stole 5 from each of ' + others.length + ' teams (+' + gain + ')';
  }
  return (u.name || pin) + ' wheel: ' + (result.label || '');
}

function fillActivateFOExtra() {
  const extra = document.getElementById('fo-activate-extra');
  if (!extra) return;
  ...
  const u = data.users[pin] || {};
  if (type === 'wheel') {
    extra.innerHTML = '<p>Spins the Wheel of Boom for <strong>' + (u.name || pin) + '</strong> and applies +points / roster 9 / steal 5 automatically.</p>';
    return;
  }
  if (type === 'forced_trade') {
    const others = Object.entries(data.users).filter(([p, x]) => p !== pin && x.division === u.division && !x.purged);
    extra.innerHTML =
      '<label class="block text-xs text-gray-400 mb-1">Take from</label>' +
      '<select id="fo-ft-target" class="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm mb-2">' +
      '<option value="">Target team...</option>' +
      others.map(([p, x]) => '<option value="' + p + '">' + (x.name || p) + '</option>').join('') +
      '</select>' +
      '<select id="fo-ft-take" class="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm mb-2"><option value="">Wrestler to take...</option></select>' +
      '<select id="fo-ft-give" class="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm mb-2">' +
      '<option value="">Wrestler to give...</option>' +
      (u.roster || []).map(w => '<option value="' + w.replace(/"/g, '&quot;') + '">' + w + '</option>').join('') +
      '</select>';
    extra.querySelector('#fo-ft-target')?.addEventListener('change', (e) => {
      const t = data.users[e.target.value];
      const sel = extra.querySelector('#fo-ft-take');
      if (!sel) return;
      sel.innerHTML = '<option value="">Wrestler to take...</option>' +
        ((t && t.roster) || []).map(w => '<option value="' + w.replace(/"/g, '&quot;') + '">' + w + '</option>').join('');
    });
    return;
  }
  if (type === 'waiver_bank') {
    const taken = new Set();
    Object.values(data.users).forEach(x => (x.roster || []).forEach(w => taken.add(String(w).toLowerCase())));
    const fa = (data.masterRoster || []).filter(n => !taken.has(String(n).toLowerCase()));
    extra.innerHTML =
      '<p class="mb-2 text-xs">Instant pickup. Ignores waiver order.</p>' +
      '<select id="fo-wb-add" class="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm mb-2">' +
      '<option value="">Add wrestler...</option>' +
      fa.map(n => '<option value="' + n.replace(/"/g, '&quot;') + '">' + n + '</option>').join('') +
      '</select>' +
      '<select id="fo-wb-drop" class="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm mb-2">' +
      '<option value="">Drop wrestler...</option>' +
      (u.roster || []).map(w => '<option value="' + w.replace(/"/g, '&quot;') + '">' + w + '</option>').join('') +
      '</select>';
    return;
  }
  if (type === 'ppv_week') {
    extra.innerHTML = '<p>Next scored show uses <strong>PPV points</strong> for ' + (u.name || pin) + ' even if it is TV.</p>';
    return;
  }
  if (type === 'waiver_disrupter') {
    extra.innerHTML = '<p>Marks this player to fire the Waiver Disrupter on the next waiver run.</p>';
    return;
  }
}

async function activateForeignObject() {
  if (typeof isReadOnlyViewer === 'function' && isReadOnlyViewer()) {
    alert('View only');
    return;
  }
  ...
  const type = document.getElementById('fo-activate-type')?.value;
  if (!pin || !type) {
    alert('Select the player, then their Foreign Object');
    return;
  }
  const name = data.users[pin]?.name || pin;
  const foName = FO_TYPES[type]?.name || type;
  const sb = window.supabaseClient;
  try {
    if (type === 'wheel') {
      if (!confirm('Spin Wheel of Boom for ' + name + ' and apply the result now?')) return;
      const randomIndex = Math.floor(Math.random() * WHEEL_SEGMENTS.length);
      const result = WHEEL_SEGMENTS[randomIndex];
      const msg = await applyWheelSegment(pin, result);
      foLog(msg);
      alert(msg);
    } else if (type === 'forced_trade') {
      const target = document.getElementById('fo-ft-target')?.value;
      const take = document.getElementById('fo-ft-take')?.value;
      const give = document.getElementById('fo-ft-give')?.value;
      if (!target || !take || !give) return alert('Pick the other team, who to take, and who to give');
      if (!confirm('Force trade: ' + name + ' gives ' + give + ' for ' + take + '?')) return;
      const a = (data.users[pin].roster || []).filter(w => w !== give);
      a.push(take);
      const b = (data.users[target].roster || []).filter(w => w !== take);
      b.push(give);
      await sb.from('teams').update({ roster: a }).eq('pin', pin);
      await sb.from('teams').update({ roster: b }).eq('pin', target);
      data.users[pin].roster = a;
      data.users[target].roster = b;
      foLog('Forced trade: ' + name + ' got ' + take + ', sent ' + give);
    } else if (type === 'waiver_bank') {
      const add = document.getElementById('fo-wb-add')?.value;
      const drop = document.getElementById('fo-wb-drop')?.value;
      if (!add || !drop) return alert('Pick who to add and who to drop');
      if (!confirm(name + ' drops ' + drop + ' for ' + add + ' now?')) return;
      const roster = (data.users[pin].roster || []).filter(w => w !== drop);
      roster.push(add);
      await sb.from('teams').update({ roster }).eq('pin', pin);
      data.users[pin].roster = roster;
      foLog('Waiver In The Bank: ' + name + ' +' + add + ' / -' + drop);
    } else if (type === 'ppv_week') {
      if (!confirm('Next show ' + name + ' scores at PPV rates?')) return;
      await sb.from('settings').upsert({ key: 'ppv_week_pin', value: pin });
      foLog('PPV Week armed for ' + name);
    } else if (type === 'waiver_disrupter') {
      if (!confirm('Arm Waiver Disrupter for ' + name + ' on the next waiver run?')) return;
      await sb.from('settings').upsert({ key: 'waiver_disrupter_pin', value: pin });
      foLog('Waiver Disrupter armed for ' + name);
    } else {
      return alert('Unknown FO');
    }
    const res = await removeHeldFO(pin, type);
    if (!res.ok) alert('Effect applied but could not clear the held FO: ' + res.error);
    renderForeignObjects();
    if (typeof renderStandings === 'function') renderStandings();
    if (typeof renderMyTeam === 'function') renderMyTeam();
  } catch (e) {
    alert('FO failed: ' + (e.message || e));
  }
}
...
function renderForeignObjects() {
  ...
  if (heldEl) {
    ...
    Object.entries(data.foreignObjects || {}).forEach(([pin, list]) => {
      ...
      if (!u) return;
      (list || []).forEach(fo => {
        entries.push(`<div class="flex justify-between items-center gap-2 text-sm">
          <span>${u.name}</span>
          <span class="text-aew-gold">${fo.name || fo.id}${fo.banked ? ' (banked)' : ''}</span>
          <button type="button" class="fo-clear-held text-[10px] bg-red-800 hover:bg-red-700 text-white font-bold px-2 py-0.5 rounded" data-pin="${pin}" data-fo="${fo.id || fo.fo_type}">Clear</button>
        </div>`);
      });
    });
...
async function ackCurrentAlert() {
  ...
  const cur = (pendingBroadcasts || []).find(b => b.id === currentAlertId);
  const bulk = /SHOW RESULTS|SCRAPE|WAIVER WIRE AUTO-RUN/i.test(String(cur && cur.message || ''));
  const ids = bulk
    ? (pendingBroadcasts || []).filter(b => /SHOW RESULTS|SCRAPE|WAIVER WIRE AUTO-RUN/i.test(String(b.message || ''))).map(b => b.id)
    : [currentAlertId];
  for (const id of ids) {
  ...
  pendingBroadcasts = pendingBroadcasts.filter(b => !ids.includes(b.id));
  currentAlertId = null;
  showNextAlert();
}
...
async function softRefreshAfterImportHook() {
  try {
    /* scrape auto-invoke disabled — cron + login were stacking RED ALERTs */
...
async function softRefresh(showToast = false) {
  ...
  purgeCheckInterval = setInterval(() => {
    ...
    await loadPendingBroadcasts(); pollWheelLive();
    await updateDraftScheduleBanner();
    updateTradeBanner();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Login button + Enter-key binding lives in the "emergency login" block
  // near the bottom of this file (window.tryLogin). It used to also be
  // bound here, which meant every click ran login() -> loadAllData() ->
  // enterApp() TWICE (including the commissioner's show-results draft
  // check firing twice per login). Don't rebind it here.
  ...
  document.getElementById('wheel-live-ok')?.addEventListener('click', () => {
    document.getElementById('wheel-live-modal')?.classList.add('hidden');
  });
  document.getElementById('wheel-replay-btn')?.addEventListener('click', replayLastWheel);
  ...
  document.getElementById('fo-activate-player')?.addEventListener('change', () => { fillActivateFOTypes(); fillActivateFOExtra(); });
  document.getElementById('fo-activate-type')?.addEventListener('change', fillActivateFOExtra);
  document.getElementById('fo-activate-btn')?.addEventListener('click', activateForeignObject);
  document.getElementById('fo-held-list')?.addEventListener('click', async (e) => {
    const btn = e.target.closest('.fo-clear-held');
    if (!btn) return;
    const pin = btn.dataset.pin;
    const type = btn.dataset.fo;
    const name = data.users[pin]?.name || pin;
    if (!confirm('Clear ' + (FO_TYPES[type]?.name || type) + ' from ' + name + '?')) return;
    const res = await removeHeldFO(pin, type);
    if (!res.ok) return alert('Could not clear: ' + res.error);
    renderForeignObjects();
  });
  ...
  document.getElementById('rescind-entire-claim')?.addEventListener('click', rescindEntireClaim);
  ...
  document.getElementById('view-pending-trades')?.addEventListener('click', () => {
    if (data.users[currentUser]?.isCommissioner) {
      showTab('commissioner');
      document.getElementById('commish-trade-queue')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } else {
...
async function clearPendingShowDraft() {
  ...
  try {
    await window.supabaseClient.from('settings').upsert({ key: 'pending_show_import', value: '' });
  } catch (e) {}
  try {
    const meta = currentShowImport && currentShowImport.meta;
    const stamp = (meta && (meta.showDate || '') + '|' + (meta.showLabel || '')) || new Date().toISOString().slice(0,10);
    await window.supabaseClient.from('settings').upsert({ key: 'dismissed_show_import', value: stamp });
  } catch (e) {}
}
...
async function loadPendingShowDraft() {
  ...
  try {
    const { data: row } = await window.supabaseClient
      ...
      .maybeSingle();
    if (!row?.value || row.value === '' || row.value === '{}') return null;
...
async function promptCommissionerPendingDraft() {
  ...
  try {
    const { data: drow } = await window.supabaseClient.from('settings').select('value').eq('key', 'dismissed_show_import').maybeSingle();
    const stamp = ((pending.meta && pending.meta.showDate) || '') + '|' + ((pending.meta && pending.meta.showLabel) || '');
    if (drow && drow.value && drow.value === stamp) return;
  } catch (e) {}
...
function openShowImportModal(draft) {
  ...
  mEl.innerHTML = draft.matches.map((m, i) => {
    ...
    let fallControl = '';
    if (m.is_multi && (m.winners || []).length) {
      // Recaps rarely state who scored the fall in a scrapeable way, so this
      // is only ever a guess (defaults to the first parsed winner). Never
      // trust it silently — the commissioner confirms/corrects it here,
      // since it drives the "end multi-person match" bonus.
      const opts = m.winners.map(w =>
        `<option value="${w.replace(/"/g, '&quot;')}" ${w === m.fall_winner ? 'selected' : ''}>${w}</option>`
      ).join('');
      fallControl = ` · <span class="text-amber-400">pin:</span>
        <select class="sim-fall-select bg-black border border-amber-700 rounded px-1 py-0.5 text-xs" data-idx="${i}">${opts}</select>
        <span class="text-amber-500" title="Auto-detected guess — confirm this is right">(confirm)</span>`;
    }
    return `<div class="border border-gray-800 rounded-lg p-2">
      ...
      <div class="text-xs text-gray-500 mt-0.5">${m.is_multi ? 'multi' : 'singles'}${fallControl}${m.tournament_round && m.tournament_round !== 'none' ? ' · tournament:' + m.tournament_round : ''}</div>
    </div>`;
  }).join('') || '<p class="text-gray-500">No matches parsed</p>';

  mEl.querySelectorAll('.sim-fall-select').forEach(sel => {
    sel.addEventListener('change', (e) => {
      updateFallWinner(Number(e.target.dataset.idx), e.target.value);
    });
  });
...
/** Commissioner corrected who actually scored the fall in a multi-person/tag
 *  match — recompute points from the match data (the "end multi-person
 *  match" bonus moves off the old guess and onto the confirmed wrestler),
 *  but keep any totals the commissioner already hand-edited this session. */
function updateFallWinner(matchIdx, name) {
  if (!currentShowImport || !currentShowImport.matches[matchIdx]) return;
  const match = currentShowImport.matches[matchIdx];
  match.fall_winner = name;

  // Preserve manual point overrides EXCEPT for wrestlers in this match —
  // their totals need to reflect the fall-winner change we just made, or a
  // stale override from earlier in the review would silently cancel it
  // back out with no indication anything reverted.
  const affected = new Set((match.winners || []).map(w => w.toLowerCase()));
  const prevOverrides = {};
  Object.entries(currentShowImport.pointsMap || {}).forEach(([n, p]) => {
    if (p.override && !affected.has(n.toLowerCase())) prevOverrides[n] = p;
  });

  const recomputed = buildImportDraft(currentShowImport.matches, currentShowImport.meta);
  Object.entries(prevOverrides).forEach(([n, p]) => {
    if (recomputed.pointsMap[n]) recomputed.pointsMap[n] = p;
  });

  currentShowImport.pointsMap = recomputed.pointsMap;
  currentShowImport.playerBonuses = recomputed.playerBonuses;
  openShowImportModal(currentShowImport);
}
...
let _showImportUIWired = false;
function wireShowImportUI() {
  // Called from two places below (immediate + DOMContentLoaded) so the DOM
  // is definitely ready either way — but without this guard, that meant
  // every listener here got bound TWICE, including sim-approve. A single
  // click on Approve was calling approveShowImport() twice, which doubled
  // every wrestler's points and every team's score on every approval.
  if (_showImportUIWired) return;
  _showImportUIWired = true;
  ...
  document.getElementById('sim-reject')?.addEventListener('click', async () => {
    closeShowImportModal();
    const draft = currentShowImport;
    currentShowImport = draft;
    await clearPendingShowDraft();
    currentShowImport = null;
    closeShowImportModal();
  });
  document.getElementById('sim-close-x')?.addEventListener('click', async () => {
    closeShowImportModal();
    await clearPendingShowDraft();
    currentShowImport = null;
  });
