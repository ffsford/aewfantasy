...
async function loadSeasonDates() {
  ...
  data.seasonEnd = null;
  try {
    const { data: rows } = await window.supabaseClient.from('settings').select('key, value').in('key', ['season_start', 'season_end']);
    (rows || []).forEach(r => {
...
async function addCalendarEvent(ev) {
  ...
  const { data: inserted, error } = await window.supabaseClient.from('calendar_events').insert(row).select().maybeSingle();
  if (error) throw error;
  if (inserted) data.calendarEvents.push(inserted);
...
async function updateCalendarEvent(id, ev) {
  ...
  const { error } = await window.supabaseClient.from('calendar_events').update(row).eq('id', id);
  if (error) throw error;
  const i = (data.calendarEvents || []).findIndex(e => e.id === id);
  if (i >= 0) data.calendarEvents[i] = { ...data.calendarEvents[i], ...row, id };
  else await loadCalendarEvents();
  await loadTradeBlocks();
}
...
function renderCalendar() {
  ...
  (data.calendarEvents || []).forEach(ev => {
    if (ev.frequency === 'weekly' || ev.event_type === 'weekly_tv') {
      weekly.push(ev);
  ...
  const card = (ev, extra) => `
    <div class="bg-aew-card rounded-xl border border-gray-800 px-4 py-3 flex flex-wrap justify-between gap-2 items-start">
      <div>
        <div class="font-semibold">${ev.name || 'Untitled'}</div>
        <div class="text-xs text-gray-400 mt-0.5">${CAL_TYPES[ev.event_type] || ev.event_type}${extra ? ' · ' + extra : ''}</div>
        ${ev.notes ? `<div class="text-xs text-gray-500 mt-1">${ev.notes}</div>` : ''}
      </div>
    </div>`;

  const wEl = document.getElementById('cal-weekly');
  if (wEl) {
    wEl.innerHTML = weekly.length
      ? weekly.map(ev => card(ev, ev.day_of_week != null ? DOW[Number(ev.day_of_week)] + 's' : '')).join('')
  ...
  if (uEl) {
    uEl.innerHTML = upcoming.length
      ? upcoming.map(ev => {
          ...
          const label = d && !isNaN(d.getTime()) ? d.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }) : (ev.event_date || '');
          return card(ev, label);
        }).join('')
      : '<p class="text-gray-500 text-sm">None yet</p>';
  }
  const oEl = document.getElementById('cal-other');
  if (oEl) {
    oEl.innerHTML = other.length
      ? other.map(ev => card(ev, '')).join('')
      : '<p class="text-gray-500 text-sm">None yet</p>';
  }
}
...
function renderCalendarAdmin() {
  ...
  list.innerHTML = events.map(ev => {
    const extra = ev.frequency === 'weekly' && ev.day_of_week != null
      ? DOW[Number(ev.day_of_week)] + 's'
      : (ev.event_date || '');
    ...
    return `<div class="flex justify-between gap-2 items-center bg-black/40 rounded-lg px-3 py-2">
      <div class="min-w-0">
        ...
        <div class="text-xs text-gray-500">${CAL_TYPES[ev.event_type] || ev.event_type} · ${ev.frequency || ''}${extra ? ' · ' + extra : ''}${noteBit}</div>
      </div>
...
async function renameWrestlerEverywhere(fromName, toName) {
  ...
  // claims ranked lists
  try {
    const { data: claims } = await sb.from('claims').select('pin, ranked, drop');
...
function renderStandings() {
  ...
  if (notice) {
    if (data.nextPurgeDate) {
      const d = new Date(data.nextPurgeDate + 'T12:00:00');
      const label = isNaN(d.getTime()) ? data.nextPurgeDate : d.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
      notice.innerHTML = `<span class="text-red-400 font-bold">PURGE</span> scheduled for <strong>${label}</strong>${data.purgeNotes ? ' — ' + data.purgeNotes : ''}. Last place in each division is removed; rosters go to free agency.`;
    ...
    el.innerHTML = list.map((t, i) => {
      ...
      <div class="standing-row ${t.pin === currentUser ? 'me' : ''}">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <span class="font-semibold truncate ${t.purged ? 'line-through text-red-400/80' : ''}">${t.name}</span>${t.purged ? '<span class="text-[10px] font-bold uppercase tracking-wide text-red-500 border border-red-500/60 rounded px-1.5 py-0.5 ml-1.5 flex-shrink-0">PURGED</span>' : ''}${t.purged ? '' : champCrown(t.pin)}
...
async function processWaivers() {
  if (claimsAreSealed()) {
    alert('Waiver claims are sealed until the deadline. You cannot process or preview claims before ' + new Date(data.waiverDeadline).toLocaleString() + '.');
    return;
  }
...
async function enterApp() {
  ...
  const u = data.users[currentUser];
  const badge = u.name + (u.isCommissioner ? ' ⭐' : '') + (isReadOnlyViewer() ? ' (view only)' : '');
...
async function renderDraft() {
  ...
  if (pickArea) {
    if (d?.status === 'active' && d.order_pins?.length) {
      ...
      const available = data.masterRoster.filter(w => !taken.has(w.toLowerCase())).sort((a,b) => getWrestlerPts(b) - getWrestlerPts(a) || a.localeCompare(b));
      ...
      pickArea.innerHTML = `
        ...
        ${isMyTurn || isCommish ? `
          <div class="max-h-80 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
            ${available.slice(0, 120).map(w => `
              <button class="draft-pick-btn text-left text-sm px-2 py-2 rounded bg-black/50 hover:bg-gray-800 border border-gray-800 flex items-center gap-2" data-name="${w}">
                ${portraitHtml(w, 'h-14 w-14')}
                <span class="truncate flex-1">${w}</span>
                <span class="text-gray-500 font-mono text-xs">${getWrestlerPts(w)}</span>
              </button>
            `).join('')}
          </div>
...
function renderScoreInputs() {
  ...
  const row = ([pin, u]) => {
    ...
    <div class="flex items-center gap-2 flex-wrap py-1">
      <span class="text-sm w-28 font-medium">${u.name}</span>
...
document.addEventListener('DOMContentLoaded', async () => {
  // Avoid login flash when session exists
  const savedPinEarly = localStorage.getItem('aew_current_pin');
  if (savedPinEarly) {
    document.getElementById('login-screen')?.classList.add('hidden');
    // optional: keep app hidden until data ready
  }
  ...
  if (savedPin && data.users[savedPin]) {
    ...
    enterApp();
  } else if (savedPinEarly) {
    // PIN stored but not in teams — show login again
    document.getElementById('login-screen')?.classList.remove('hidden');
  }
  ...
  document.getElementById('submit-claim')?.addEventListener('click', async () => {
    ...
    document.getElementById('claim-status').textContent = claimsAreSealed()
      ? 'Claim locked in. Hidden from everyone until the deadline (' + new Date(data.waiverDeadline).toLocaleString() + ').'
      : 'Claim submitted successfully (drop: ' + drop + ')';
  ...
  document.getElementById('cal-ev-add')?.addEventListener('click', async () => {
    ...
    const frequency = document.getElementById('cal-ev-freq')?.value || 'once';
    const day_of_week = frequency === 'weekly' ? parseInt(document.getElementById('cal-ev-day')?.value, 10) : null;
    const event_date = frequency === 'once' ? (document.getElementById('cal-ev-date')?.value || null) : (document.getElementById('cal-ev-date')?.value || null);
    ...
    const editId = document.getElementById('cal-ev-id')?.value || '';
    try {
      if (editId) {
        await updateCalendarEvent(editId, { name, event_type, frequency, day_of_week: isNaN(day_of_week) ? null : day_of_week, event_date, notes });
        if (msg) { msg.textContent = 'Event updated'; msg.className = 'text-sm text-emerald-400'; }
      } else {
        await addCalendarEvent({ name, event_type, frequency, day_of_week: isNaN(day_of_week) ? null : day_of_week, event_date, notes });
        if (msg) { msg.textContent = 'Event added'; msg.className = 'text-sm text-emerald-400'; }
      }
      clearCalEventForm();
      renderCalendarAdmin();
      renderCalendar();
    } catch (e) {
      if (msg) { msg.textContent = 'Error: ' + (e.message || e) + ' — run calendar_events SQL if needed'; msg.className = 'text-sm text-red-400'; }
    }
  });
  ...
  document.getElementById('purge-schedule-save')?.addEventListener('click', async () => {
    ...
    await savePurgeSchedule(d, n);
    const msg = document.getElementById('purge-schedule-msg');
    if (msg) { msg.textContent = d ? 'Purge scheduled for ' + d : 'Purge schedule cleared'; msg.className = 'text-sm mb-4 text-emerald-400'; }
    renderStandings();
  });
