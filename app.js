...
function teamNameEditInProgress() {
  const inp = document.getElementById('my-team-name-input');
  if (!inp) return false;
  if (document.activeElement === inp) return true;
  const saved = String(data.users[currentUser]?.teamName || '');
  return String(inp.value || '') !== saved;
}
...
function renderMyTeam() {
  ...
  if (!u) return;
  const nameInp = document.getElementById('my-team-name-input');
  const keepName = (nameInp && document.activeElement === nameInp) ? { v: nameInp.value, s: nameInp.selectionStart, e: nameInp.selectionEnd } : null;
  ...
  if (keepName) {
    const inp = document.getElementById('my-team-name-input');
    if (inp) {
      inp.value = keepName.v;
      inp.focus();
      try { inp.setSelectionRange(keepName.s, keepName.e); } catch (_) {}
    }
  }
}
...
async function softRefresh(showToast = false) {
  ...
  purgeCheckInterval = setInterval(() => {
    ...
    else if (tab === 'myteam') { if (!teamNameEditInProgress()) renderMyTeam(); }
