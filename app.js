...
function renderDropPool() {
  ...
  const roster = data.users[currentUser]?.roster || [];
  el.innerHTML = `
    <div class="text-xs text-gray-400 mb-2">Players you are willing to drop (max pickups = number of drops). Order does not matter.</div>
    <div class="flex flex-wrap gap-2 mb-2" id="drop-pool-chips">
      ...
      `).join('') || '<span class="text-gray-500 text-sm">No drops selected yet</span>'}
    </div>
    <select id="add-drop-pool-select" class="w-full bg-black border border-gray-600 rounded-lg px-3 py-2 text-sm">
      <option value="">+ Add a drop from your roster…</option>
      ${roster.filter(r => !claimDrops.some(d => (d||'').toLowerCase() === (r||'').toLowerCase())).map(r =>
        `<option value="${String(r).replace(/"/g,'&quot;')}">${r}</option>`
      ).join('')}
    </select>
    <p class="text-[11px] text-gray-500 mt-2">Example: rank Gabe, Toa, Clark + drop Daniel &amp; Tommaso → you get up to <strong class="text-gray-300">2</strong> of those three (first available in rank order).</p>
  `;
...
function renderClaimPreview() {
  ...
  prev.innerHTML = `
    <div class="rounded-lg border border-gray-700 p-3 text-sm space-y-1">
      ...
      <div class="text-aew-gold text-xs mt-1">Max pickups this period: <strong>${n}</strong> (fills top available names in order)</div>
    </div>
  `;
}
...
function renderClaimList() {
  ...
  if (claimRanked.length === 0) {
    el.innerHTML = '<p class="text-sm text-gray-500 text-center py-6">Tap available wrestlers to rank who you want (priority order, up to 8).</p>';
  } else {
