...
async function persistClaimRanked() {
  ...
  const drop = document.getElementById('drop-select')?.value || data.claims[currentUser]?.drop || null;
  try {
    if (claimRanked.length === 0) {
      // clear saved claim if list empty
      await window.supabaseClient.from('claims').delete().eq('pin', currentUser);
      delete data.claims[currentUser];
    } else {
      const row = { pin: currentUser, ranked: claimRanked, drop: drop || null };
      const { error } = await window.supabaseClient.from('claims').upsert(row, { onConflict: 'pin' });
      if (error) throw error;
      data.claims[currentUser] = { ranked: [...claimRanked], drop: drop || null };
    }
  } catch (e) {
    console.error('persistClaimRanked', e);
    alert('Could not update claim: ' + (e.message || e));
  }
}
...
function renderClaimList() {
  ...
  el.innerHTML = claimRanked.map((w, i) => `
    <div class="claim-item flex items-center justify-between gap-2 py-1">
  ...
  el.querySelectorAll('.remove-claim').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (isNaN(idx) || idx < 0 || idx >= claimRanked.length) return;
      claimRanked.splice(idx, 1);
      renderClaimList();
      await persistClaimRanked();
      const status = document.getElementById('claim-status');
      if (status) {
        status.textContent = claimRanked.length ? 'Claim updated' : 'Claim cleared';
...
async function reloadClaimsFromDb() {
  const sb = window.supabaseClient;
  if (!sb) return;
  const { data: claims } = await sb.from('claims').select('pin, ranked, drop, submitted_at');
  data.claims = {};
  (claims || []).forEach(c => {
    data.claims[c.pin] = { ranked: c.ranked || [], drop: c.drop || null, submittedAt: c.submitted_at };
  });
...
async function tryAutoProcessWaivers() {
  ...
  try {
    const { data: row } = await window.supabaseClient
      .from('settings')
      .select('value')
      .eq('key', 'last_processed_waiver_deadline')
      .maybeSingle();
  ...
  for (const div of ['east', 'west']) {
    ...
    for (const team of order) {
      ...
      for (const wrestler of claim.ranked) {
        ...
        if (!availableHas(available, wrestler)) continue;
        ...
        if (drop && rosterHas(u.roster, drop)) {
          ...
          results.push(`⚠️ ${team.name} wanted ${claimName} but no valid drop selected / roster full`);
        }
      }
    }
  ...
  try {
    if (data.waiverDeadline) {
      await window.supabaseClient.from('settings').upsert({
        key: 'last_processed_waiver_deadline',
