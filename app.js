...
function fillCalEventForm(ev) {
  ...
  document.getElementById('cal-ev-type').value = ev.event_type || 'other';
  document.getElementById('cal-ev-freq').value = ev.frequency || 'once';
  if (ev.day_of_week != null) document.getElementById('cal-ev-day').value = String(ev.day_of_week);
  document.getElementById('cal-ev-date').value = ev.event_date || '';
...
function renderCalendarAdmin() {
  ...
  list.querySelectorAll('[data-cal-edit]').forEach(btn => {
    btn.onclick = () => {
      const ev = (data.calendarEvents || []).find(e => e.id === btn.dataset.calEdit);
      if (ev) {
        fillCalEventForm(ev);
        document.getElementById('cal-ev-name')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    };
  });
