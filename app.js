...
async function persistClaimRanked() {
  ...
  try {
    ...
    document.querySelectorAll('.claim-drop-select').forEach(sel => {
      const i = parseInt(sel.getAttribute('data-idx'), 10);
      if (!isNaN(i)) claimDrops[i] = sel.value || '';
    });
    const drop = claimDrops[0] || document.getElementById('drop-select')?.value || null;
    await saveClaim(currentUser, [...claimRanked], drop, [...claimDrops]);
  } catch (e) {
...
function renderClaimList() {
  ...
  if (!el) return;
  while (typeof claimDrops === 'undefined') { break; }
  if (typeof claimDrops === 'undefined') window.claimDrops = [];
  // align lengths
  if (!Array.isArray(claimDrops)) claimDrops = [];
  while (claimDrops.length < claimRanked.length) claimDrops.push('');
  claimDrops.length = claimRanked.length;

  if (claimRanked.length === 0) {
    el.innerHTML = '<p class="text-sm text-gray-500 text-center py-6">Tap available wrestlers to build your claim list (up to <strong class="text-white">8</strong>).<br><span class="text-gray-400">Each line = one claim + its own drop. You can drop 8 and claim 8 in one period if they all hit.</span></p>';
    return;
  }
  const roster = data.users[currentUser]?.roster || [];
  el.innerHTML = claimRanked.map((w, i) => {
    const opts = roster.map(r => {
      const sel = (claimDrops[i] || '') === r ? ' selected' : '';
      return `<option value="${String(r).replace(/"/g, '&quot;')}"${sel}>${r}</option>`;
    }).join('');
    return `
    <div class="claim-item space-y-2 py-2 border-b border-gray-800">
      <div class="flex items-center justify-between gap-2">
        <div class="flex items-center gap-2 min-w-0">
          <span class="text-aew-gold text-xs font-bold w-5">${i + 1}.</span>
          ${portraitHtml(w, 'h-16 w-16')}
          <span class="truncate font-medium">${w}</span>
        </div>
        <button type="button" class="text-red-400 text-lg leading-none px-2 py-1 remove-claim flex-shrink-0" data-idx="${i}">✕</button>
      </div>
      <div class="pl-6">
        <label class="text-[11px] text-gray-500">Drop if this claim hits:</label>
        <select class="claim-drop-select w-full mt-0.5 bg-black border border-gray-700 rounded-lg px-2 py-1.5 text-sm" data-idx="${i}">
  ...
  el.querySelectorAll('.remove-claim').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (isNaN(i) || i < 0 || i >= claimRanked.length) return;
      claimRanked.splice(i, 1);
      claimDrops.splice(i, 1);
      renderClaimList();
      try { await persistClaimRanked(); } catch (_) {}
    });
  });
  el.querySelectorAll('.claim-drop-select').forEach(sel => {
    ...
    sel.addEventListener('change', (e) => {
      ...
      const i = parseInt(sel.getAttribute('data-idx'), 10);
      if (!isNaN(i)) {
        claimDrops[i] = sel.value || '';
        // Do NOT re-render full waiver — keeps list visible
      }
    });
  });
}
...
async function processWaivers(opts = {}) {
  ...
  for (const div of ['east', 'west']) {
    ...
    for (const team of order) {
      ...
      // Each ranked line is its own claim with its own drop.
      // Process #1, #2, #3... — skip if target taken or drop already used; can hit MULTIPLE per team.
      const dropsArr = Array.isArray(claim.drops) && claim.drops.length
        ? claim.drops
        : (claim.drop ? claim.ranked.map(() => claim.drop) : []);
      const usedDrops = new Set();
      let hits = 0;
      for (let ri = 0; ri < claim.ranked.length; ri++) {
        const wrestler = claim.ranked[ri];
        const drop = dropsArr[ri] || claim.drop || null;
        const available = getAvailableWrestlers(div);
        if (!availableHas(available, wrestler)) {
          results.push(`↪️ ${team.name} #${ri + 1} ${wrestler} taken — skip`);
          continue;
        }
        if (drop && usedDrops.has(String(drop).toLowerCase())) {
          results.push(`↪️ ${team.name} #${ri + 1} skip — drop ${drop} already used this run`);
          continue;
        }
        const claimName = availableCanonical(available, wrestler);
        const u = data.users[team.pin];
        u.roster = u.roster || [];
        const maxR = u.maxRoster || 8;

        if (drop && rosterHas(u.roster, drop)) {
          u.roster = rosterWithout(u.roster, drop);
          u.roster.push(claimName);
          await saveTeamRoster(team.pin, u.roster);
          usedDrops.add(String(drop).toLowerCase());
          hits++;
          results.push(`✅ ${team.name} claimed <strong>${claimName}</strong> (dropped ${drop})`);
        } else if (!drop && u.roster.length < maxR) {
          u.roster.push(claimName);
          await saveTeamRoster(team.pin, u.roster);
          hits++;
          results.push(`✅ ${team.name} claimed <strong>${claimName}</strong>`);
        } else {
          results.push(`⚠️ ${team.name} #${ri + 1} ${claimName} — drop "${drop || 'none'}" invalid or roster full`);
        }
      }
      await clearClaim(team.pin);
      if (!hits) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('submit-claim')?.addEventListener('click', async () => {
    ...
    const missing = claimRanked.findIndex((_, i) => !claimDrops[i]);
    if (missing >= 0) {
      document.getElementById('claim-status').textContent = 'Select a drop for claim #' + (missing + 1) + ' (' + claimRanked[missing] + ')';
      document.getElementById('claim-status').className = 'text-sm text-center mt-3 text-red-400';
      return;
    }
    // same drop used twice is OK only if different claims can't both hit same roster seat — warn if duplicate drops
    const dropSet = new Set(claimDrops);
    if (dropSet.size < claimDrops.length) {
      if (!confirm('You selected the same drop on more than one claim. Only the first successful claim can use that player. Continue?')) return;
    }
    try {
      await saveClaim(currentUser, [...claimRanked], claimDrops[0], [...claimDrops]);
      document.getElementById('claim-status').textContent = claimsAreSealed()
        ? 'Claim saved & locked until deadline. Each line can hit separately (drop per line).'
        : 'Claim saved. Each ranked line is its own claim with its own drop.';
